# 离线转写开发待办清单
  ...
  - [x] 9. 实现离线依赖检查 UI 与 IPC
      - 提示词：
        ...
        检查清单：
          ...
          - [x] UI 关闭后资源释放（事件解绑）
          - [x] UI 模态/按钮/建议文字
          - [x] deps:check IPC 与依赖检测逻辑
          - [x] app:runSetupOffline IPC 后台执行脚本并回写日志
          - [x] Renderer 显示/刷新依赖状态、调用脚本、提供提示
  ...
  - [ ] 10. 写入作业日志与诊断导出
      - 提示词：
        ...
        检查清单：
          - [x] 每个作业日志文件至少追加五种类型的日志（info/error等）
          - [x] 导出文件包含必要元数据且大小合理
          - [x] 无作业时导出操作提示用户
          - [x] 阶段化日志写入（包含 start/complete/progress/error）
          - [x] 日志、元数据、产物统一存放在下载目录下的作业子目录
          - [x] 诊断导出 API 生成 zip/tar，含系统信息和关键文件
          - [x] Renderer 提供诊断导出入口并消费结果/错误回调